...
When documents appear inconsistent, preserve product intent first, note the inconsistency in the PR, and update the relevant authoritative document when the user's instruction resolves it.
...
For documentation/design-system changes not tied to a numbered backlog item, use a descriptive `agent/docs-...` branch.
...
If product/design direction changes after an older backlog item was completed, do not rewrite history by unchecking the old item. Add a new refinement item describing the delta from current behavior.
...
- Watch explanations use contextual overlays/popovers rather than a permanent third information panel.
...
- Grammar, nuance, morphology, and examples use progressive disclosure.
- Review preserves original video/source context where practical.
